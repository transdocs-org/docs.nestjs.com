...
> ⚠️ 注意 **注意** 对于你选择的 **任何** Passport 策略，你始终需要 `@nestjs/passport` 和 `passport` 包。然后，你需要安装实现特定身份验证策略的包（例如 `passport-jwt` 或 `passport-local`）。此外，你还可以安装任何 Passport 策略的类型定义，如上面的 `@types/passport-local`，这将帮助你在编写 TypeScript 代码时获得更好的支持。
...
> ⚠️ 警告 当然，在真实应用中，你不应该以明文形式存储密码。你应该使用像 [bcrypt](https://github.com/kelektiv/node.bcrypt.js#readme) 这样的库，使用加盐的单向哈希算法。使用这种方法，你只存储哈希后的密码，然后将存储的密码与传入密码的哈希版本进行比较，从而永远不会以明文形式存储或暴露用户密码。为了简化我们的示例应用，我们违反了这一绝对要求，使用了明文密码。**不要在你的真实应用中这样做！**
...
> 💡 提示 我们可以在调用 `super()` 时传递一个选项对象来定制 Passport 策略的行为。在此示例中，默认情况下，passport-local 策略期望请求体中包含名为 `username` 和 `password` 的属性。传递一个选项对象以指定不同的属性名，例如：`super({{ '{' }} usernameField: 'email' {{ '}' }})`。有关更多信息，请参阅 [Passport 文档](http://www.passportjs.org/docs/configure/)。
...
我们需要配置我们的 `AuthModule` 以使用我们刚刚定义的 Passport 特性。更新 `auth.module.ts` 文件如下所示：
...
> ⚠️ 警告 **不要公开此密钥**。我们在这里这样做是为了清楚地展示代码在做什么，但在生产系统中 **你必须使用适当的措施保护此密钥**，例如使用密钥保险库、环境变量或配置服务。
...
> 💡 提示 `ModuleRef` 类从 `@nestjs/core` 包中导入。
